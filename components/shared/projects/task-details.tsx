...
export default function TaskDetails({
  ...
  const handlePerformerSelect = (performerId: string) => {
    ...
    const numericId = performerId;
    if (!currentPerformers.includes(numericId)) {
      handleChange(
        "performers",
        JSON.stringify([...currentPerformers, numericId])
      );
    } else {
      handleChange(
        "performers",
        JSON.stringify(
          currentPerformers.filter((id: string) => id !== numericId)
        )
      );
    }
  };
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="bg-gray-800 text-green-400 border-0 sm:max-w-[550px] max-h-[90vh] overflow-hidden">
          ...
          {isEditing ? (
            ...
            >
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {JSON.parse(editedTask.performers).map(
                    (performerId: string) => {
                      ...
                      return user ? (
                        ...
                        >
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handlePerformerSelect(user.id.toString())
                            }
            ...
            <div className="space-y-6">
              ...
              <div>
                <Badge
                  variant="secondary"
                  className="bg-gray-700 text-green-400 text-xs"
                >
                  {task.taskStatus}
                </Badge>
              </div>
        ...
        {task.authorId == user.id ? (
          <DialogFooter className="justify-start w-full ">
            {isEditing ? (
              <Button
                onClick={handleSave}
