...
import { Search, ArrowLeft, ArrowRight, Check, X, FolderKanban, Clock, Eye, ListTodo } from "lucide-react"
...
// Функция для обновления статуса проекта (нужно реализовать в actions.ts)
const updateProjectStatus = async (projectId, status) => {
  // Заглушка - в реальном приложении заменить на API-вызов
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim() !== "") {
      ...
      filtered = filtered.filter(
        (project) => project.name?.toLowerCase().includes(query) || project.content?.toLowerCase().includes(query),
      )
    }
  ...
  const handleApprove = async (projectId) => {
    try {
      ...
      // Update local state
      setProjects(
        projects.map((project) => (project.id === projectId ? { ...project, projectStatus: "APPROVED" } : project)),
      )
  ...
  const handleReject = async (projectId) => {
    try {
      ...
      // Update local state
      setProjects(
        projects.map((project) => (project.id === projectId ? { ...project, projectStatus: "REJECTED" } : project)),
      )
  ...
  return (
    <div className="max-w-[1800px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-purple-400 mb-2">Модерация проектов</h1>
          <p className="text-gray-400 text-sm">Просмотр и управление проектами пользователей</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <Card className="bg-gray-800 border-gray-700 text-white">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-400">Всего проектов</p>
        ...
        <Card className="bg-gray-800 border-gray-700 text-white">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-400">Активных проектов</p>
          ...
          {totalPages > 1 && (
            <div className="flex justify-between items-center mt-8">
              <div className="text-sm text-gray-400">
                Показано {indexOfFirstProject + 1}-{Math.min(indexOfLastProject, filteredProjects.length)} из{" "}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-gray-800 border-gray-700 text-white max-w-4xl max-h-[90vh] overflow-auto">
          {selectedProject && (
            <>
              <DialogHeader>
                <DialogTitle className="text-xl text-purple-400">{selectedProject.name}</DialogTitle>
                <DialogDescription className="text-gray-400">Детальная информация о проекте</DialogDescription>
              </DialogHeader>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {getAuthor(selectedProject.authorId).firstName} {getAuthor(selectedProject.authorId).lastName}
                      </div>
                    </div>
                  </div>
                ...
                <div className="mb-6">
                  ...
                  {loadingTasks ? (
                    ...
                    <div className="bg-gray-900 rounded-md p-6 text-center">
                      <p className="text-gray-400">У этого проекта пока нет задач</p>
                    </div>
                  ) : (
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="border-b border-gray-700">
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">Задача</th>
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">Статус</th>
                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-400">Приоритет</th>
                          </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr key={task.id} className="border-b border-gray-700 last:border-0">
                              ...
                              <td className="px-4 py-3">
                                {task.status === "COMPLETED" || task.status === "DONE" ? (
                                  ...
                                  <Badge className="bg-yellow-500">Ожидает</Badge>
                                )}
                              </td>
...
function ProjectCard({ project, author, onApprove, onReject, onViewDetails }) {
  const formattedDate = new Date(project.createdAt).toLocaleDateString("ru-RU")
  const daysSinceCreation = Math.floor((new Date() - new Date(project.createdAt)) / (1000 * 60 * 60 * 24))

  return (
    <Card className="bg-gray-800 border-gray-700 text-white h-full flex flex-col">
      <CardContent className="p-6 flex flex-col flex-1">
        <div className="flex justify-between items-start gap-4 mb-4">
          <h3 className="text-xl font-bold text-purple-400 line-clamp-2">{project.name}</h3>
        ...
        <div className="mb-4 flex-1">
          <p className="text-gray-300 line-clamp-3">{project.content}</p>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto">
          <Button variant="outline" size="sm" className="gap-2" onClick={() => onViewDetails(project)}>
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => onApprove(project.id)}>
        ...
        {daysSinceCreation > 2 && (
          <Badge variant="outline" className="mt-4 self-start bg-red-400/10 text-red-400 border-red-400/20">
