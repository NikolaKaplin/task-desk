...
import { getUsers, setAdminRights, deleteUser } from "../../../actions";
...
// Добавим функцию подтверждения удаления
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function UsersPage() {
  ...
  // В компоненте UsersPage добавим состояние для диалога подтверждения удаления
  ...
  // Добавим функцию для обработки удаления пользователя
  const handleDeleteUser = async (userId) => {
    try {
      ...
      // Если удаляемый пользователь был выбран для просмотра, закрываем диалог
      if (selectedUser && selectedUser.id === userId) {
        setDialogOpen(false);
      }
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            Просмотр и управление учетными записями пользователей системы
          </p>
        </div>
            ...
            <AlertDialogDescription className="text-gray-400">
              {userToDelete && (
                <>
                  ...
                  . Это действие нельзя отменить, и все данные пользователя
